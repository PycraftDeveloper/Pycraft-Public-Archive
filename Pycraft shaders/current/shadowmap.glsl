...
void main() {
    ...
    vec3 grass_texture = texture2D(grass_color, vec2(mod(v_text.x * repeat.x, 1.0), mod(v_text.y * repeat.y, 1.0))).rgb;
    vec3 rock_texture = texture2D(rock_color, vec2(mod(v_text.x * repeat.x, 1.0), mod(v_text.y * repeat.y, 1.0))).rgb;
